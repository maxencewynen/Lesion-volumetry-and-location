...
def load_segmentations(subject):
    ...
    """
    # Load segmentations
    aseg_img = nib.load(MAIN_DIR + "/derivatives/segmentations/sub-{0}/ses-".format(subject)+SESSION+"/sub-{0}_aseg_lesions.nii.gz".format(subject))
...
def make_lesions_csv(subject, minimum_lesion_size=5):
    ...
    df.to_csv(MAIN_DIR + "/derivatives/stats/sub-{0}/ses-".format(subject)+SESSION+"/sub-{0}_lesions.csv".format(subject))
    
    # Save the lesion mask with labels to a nifti file
    nifti_out = nib.Nifti1Image(labeled_lesions,affine=image.affine)
    nib.save(nifti_out, MAIN_DIR+'/derivatives/segmentations/sub-{0}/ses-'.format(subject)+SESSION+'/sub-{0}_labeled_lesions.nii.gz'.format(subject))
...
def lesion_location(subject, lesion_label, lesion_mx=None, aseg_mx=None, percentage=0.2):
    ...
    if lesion_mx is None:
        # Load lesion mask
        lesion_image = nib.load(MAIN_DIR + "/derivatives/segmentations/sub-{0}/ses-".format(subject)+SESSION+"/sub-{0}_labeled_lesions.nii.gz".format(subject))
        lesion_mx = lesion_image.get_fdata()
        
    if aseg_mx is None:
        # Load segmentation
        aseg_image = nib.load(MAIN_DIR + "/derivatives/segmentations/sub-{0}/ses-".format(subject)+SESSION+"/sub-{0}_aseg.mgz".format(subject))
...
def make_location_mask(subject, percentage=20):
    ...
    nib.save(nifti_out, MAIN_DIR+'/derivatives/segmentations/sub-{0}/ses-'.format(subject) +SESSION+'/sub-{0}_lesion_locations.nii.gz'.format(subject))
